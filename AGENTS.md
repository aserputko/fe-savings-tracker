...
The API client in `src/api/generated/` is generated from the backend's OpenAPI/Swagger spec using `openapi-generator-cli`.

```bash
npm run api-generate   # fetch swagger.json, clean generated/, regenerate, add // @ts-nocheck
npm run api-swagger    # only fetch swagger.json from http://localhost:4000/api/docs/json
npm run api-docker     # only run the generator (requires swagger.json to exist)
npm run api-nocheck    # only prepend // @ts-nocheck to all generated *.ts files
npm run api-clean      # delete src/api/generated/
```

- Always use `npm run api-generate` to regenerate the client — it runs all steps including `api-nocheck`.
- Never run `api-docker` alone without following up with `api-nocheck`, or the build will fail due to TypeScript errors in generated code.
...
- Generated API client: [src/api/generated/](src/api/generated/) — **do not edit manually**
...
- `src/api/generated/` is auto-generated by `openapi-generator-cli` (typescript-fetch). Never edit files in this folder manually — changes will be overwritten on next `npm run api-generate`.
- All files in `src/api/generated/` contain a `// @ts-nocheck` header to suppress TypeScript errors from generated code. This is intentional — do not remove it.
- `tsconfig.app.json` also excludes `src/api/generated` from the TypeScript project to prevent false positives in IDE tooling.
